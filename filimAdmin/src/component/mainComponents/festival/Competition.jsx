...
const Competition = ({
  ...
  const [popupForm, setPopupForm] = useState(competate.popup || defaultPopup);

  useEffect(() => {
    setPopupForm(competate.popup || defaultPopup);
  }, [competate.popup]);
  ...
  const savePopup = () => {
    setCompetate((data) => ({ ...data, popup: { ...popupForm } }));
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
                <div className="relative w-full max-w-2xl rounded-[32px] bg-white p-8 shadow-2xl" style={{ maxHeight: 'calc(100vh - 120px)', overflow: 'auto' }}>
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <h2 className="text-2xl font-bold">Edit Section 4 Popup</h2>
                      <p className="text-sm text-gray-600">Section 4 ka alag popup content.</p>
                    </div>
                  ...
                  <div className="mt-6 space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        <label htmlFor="competatePopupImageUpload" className="flex h-12 items-center justify-center rounded border border-dashed border-gray-300 bg-gray-50 text-sm text-gray-600 hover:bg-gray-100 cursor-pointer">
                          {popupImageFile ? popupImageFile.name : popupForm.image ? "Change image" : "Upload image"}
                        </label>
                        <input id="competatePopupImageUpload" type="file" accept="image/*" className="hidden" onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (!file) return;
